...
def init_db():
   try:
       with sqlite3.connect(DB_FILE) as conn:
           ...
           ''')
           # NOVO: tabela do journal (adicionada sem tocar na tabela alerts)
           cursor.execute('''
...
# ─── PROMPT ICT CACHEADO ─────────────────────────────────────────────────────
# O prompt é enorme — com cache_control a Anthropic guarda-o por 5 min
# e não cobra os tokens do prompt em chamadas seguintes dentro desse tempo.
ICT_SYSTEM_PROMPT = (
...
def analyze():
   try:
       ...
       # Prompt dinâmico (par + timeframes mudam por análise — não cacheado)
       dynamic_prompt = (
       ...
       # CACHE: system prompt cacheado — poupa tokens em análises seguidas
       ...
       # Guardar análise no journal automaticamente
...
def update_journal():
   try:
       ...
       status = data.get('status')  # 'win', 'loss', 'cancelled', 'pending'
