...
def process_pair_scalp(db_file, pair, d1_candles, exec_candles, exec_tf_label, send_telegram_fn=None):
    ...
    if not zona:
        # ── NOVO: preço saiu da zona, mas isso NÃO apaga a memória salva.
    ...
    if not entry_zone:
        # ── NOVO: se o FVG/OB normal não existe (ou já foi rompido de
        # vez), tenta achar um iFVG — o mesmo gap, mas invertido depois
        # de romper e o preço rejeitar de volta na direção do CHoCH. ──
        entry_zone = find_ifvg_after_choch(exec_candles, choch)
    if not entry_zone:
        # ── NOVO: último fallback — Breaker Block (OB rompido de vez e
        # depois respeitado na direção do CHoCH, sequência clássica ICT
        # de 3 passos). ──
    ...
    # ── NOVO: killzone deixou de ser gate — cripto tem volume real 24h.
    # Sinaliza e manda Telegram sempre que score >= 75, independente da
    # killzone (que só contou como bônus de +10 no score, se aplicável). ──
